...
For this workshop, you'll need a Google account (Gmail or Scarletmail) and a Twitter account. Freate and/or log into both now.
...
To keep things simple, we will used RStudio Cloud to run the next few examples interactively. Go to <https://rstudio.cloud> and create an account. Then, click the dropdown arrow next to the **New Project** button and select *New Project from Git Repository*. Enter the following URL to create a project based on this handout: <https://github.com/giannetti/collecting-twitter-data>. While still in RStudio Cloud, scroll down to this section, then press the little green arrow in the upper right corner of the code block below to run the set-up. 
...
On [RStudio Cloud](https://rstudio.cloud/), there is an extra step to get rtweet up and running (you won't need to do this if your are using RStudio desktop). Go to the file called `twitterapp.rds` in your Files. Follow the instructions to create a Twitter Developer App at <https://web.archive.org/web/20200510175109/https://rtweet.info/articles/auth.html>. Edit the rds file to include the name of your App and your key and token. The Rds file stores a single R object that we will read into our environment with the following line of code. 

```{r eval=FALSE}

## edit twitterapp.rds to include your Twitter app details and run this line of code
token <- readRDS("twitterapp.rds")
```
